...
// PromptWhitelist opens a native macOS input dialog for the domain/CIDR
// whitelist (comma separated; empty clears it).
func PromptWhitelist(def string) (string, error) {
	script := `text returned of (display dialog "Domains or CIDRs routed through the tunnel (comma separated). Use *.example.com for subdomains and 10.0.0.0/8 for CIDR. Empty = full tunnel." default answer "` +
		escapeAppleScript(def) + `" with title "Skyfire — domain whitelist" buttons {"Cancel", "OK"} default button "OK")`
	out, err := exec.Command("osascript", "-e", script).Output()
	if err != nil {
		return "", err
	}
